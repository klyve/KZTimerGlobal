...
//ban statements
new String:sqlglobal_insertBan[] 					= "INSERT INTO banlist_new (steamid, playername, playercountrycode, reason, stats, serverip, servername, unix_timestamp) VALUES('%s', '%s', '%s', '%s','%s','%s', '%s', '%i');";
...
public Action:Client_Mapinfo(client, args)
{
	if (IsValidClient(client))
	{
		if (g_global_ValidedMap && g_hDbGlobal != INVALID_HANDLE)
			PrintToChat(client, "%t", "mapinfo1",YELLOW,WHITE,GRAY,	YELLOW,g_szMapName,GRAY,YELLOW,g_global_szMapDifficulty,GRAY,LIGHTBLUE,g_global_szApprover,GRAY);
		else
			PrintToChat(client, "%t", "mapinfo2",YELLOW,WHITE,GRAY,YELLOW,g_szMapName,GRAY);
	}
...
{
	if (g_global_KZTimerFileSize && g_global_ValidedMap && g_global_IntegratedButtons && !g_global_VersionBlocked && !g_bGlobalBeta && g_global_EntityCheck && !g_global_SelfBuiltButtons && g_hDbGlobal != INVALID_HANDLE && g_bEnforcer && g_global_ValidFileSize && !g_bAutoTimer)
		return true;
	return false;
}
...
public db_VersionCheck()
{
	...
	g_global_KZTimerFileSize=false;
...
public sqlglobal_VersionCheckCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	if (hndl != INVALID_HANDLE)
	{
		if(SQL_HasResultSet(hndl) && SQL_FetchRow(hndl))
		{
			...
			/*new filesize_db = SQL_FetchInt(hndl, 2);
			new filesize = FileSize(sPath);
			new Float:flDiff = FloatAbs(float(filesize_db)-float(filesize));
			*/
			g_global_KZTimerFileSize = true;
		}
	}
}

public db_InsertBan(String:szSteamId[32], String:szName[64], String:szCountryCode[16], String:szReason[256], String:szStats[256])
{
	if (g_hDbGlobal == INVALID_HANDLE)
		return;
	...
	Format(szQuery, 1024, sqlglobal_insertBan, szSteamId,szName, szCountryCode, szReason, szStats, g_szServerIp, g_szServerName,UnixTimestamp);
	...
	if (g_hDbGlobal == INVALID_HANDLE)
	{
		...
		if (g_global_Disabled)
		{
			...
			if(!StrEqual(g_szMapPrefix[0],"kz") && !StrEqual(g_szMapPrefix[0],"xc") && !StrEqual(g_szMapPrefix[0],"bkz")  && !StrEqual(g_szMapPrefix[0],"kzpro"))
			{
				...
				{
					PrintToChat(client, "[%cKZ%c] %cGlobal Records disabled. Reason: KZTimer filesize check failed.",MOSSGREEN,WHITE,RED);
					return Plugin_Handled;
				}
				else
					if (g_bGlobalBeta)
